"""Script for training a model on the DynamicAviary environment.
...
def run(output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    train_env = make_vec_env(DynamicAviary,
                             env_kwargs=dict(obs=DEFAULT_OBS, act=DEFAULT_ACT),
                             n_envs=1,
                             seed=0
                             )
    eval_env = DynamicAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT)
    ...
    # This is a placeholder, you might need to adjust it
    target_reward = 500
    ...
    test_env = DynamicAviary(gui=gui,
                           obs=DEFAULT_OBS,
                           act=DEFAULT_ACT,
                           record=record_video)
